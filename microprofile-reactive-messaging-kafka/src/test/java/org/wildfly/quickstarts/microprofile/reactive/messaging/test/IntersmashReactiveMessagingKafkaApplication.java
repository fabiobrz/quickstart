...
public class IntersmashReactiveMessagingKafkaApplication implements WildflyHelmChartOpenShiftApplication {
 private static final String APP_NAME = "microprofile-reactive-messagin-kafka";

 private final HelmChartRelease release;
 private final Map<String, String> setOverrides = new HashMap<>();

 public IntersmashReactiveMessagingKafkaApplication() {
  this.release = new HelmChartRelease(loadRelease());
 }

 IntersmashReactiveMessagingKafkaApplication addSetOverride(String name, String value) {
 ...
 private HelmWildflyRelease loadRelease() {
  URL url = IntersmashReactiveMessagingKafkaApplication.class.getClassLoader().getResource("helm.yaml");
  if (url == null) {
